...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadSessionAndSemesters = async () => {
  ...
  const createSemester = async (e: React.FormEvent) => {
    ...
    // Step 2: Auto-create semester_events for start + end
    const eventInserts = [
      {
        user_id: user.id,
  ...
  const deleteSemester = async (semesterId: string) => {
    const confirmed = window.confirm('Are you sure you want to delete this semester? This cannot be undone.')
  ...
  return (
    <main className="min-h-screen bg-zinc-900 text-white p-8 max-w-3xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Welcome back, {email}!</h1>

      <section className="mb-12">
        <h2 className="text-xl font-semibold mb-4">Your Semesters</h2>
        <ul className="space-y-4">
          {semesters.map(sem => (
            <li key={sem.id} className="bg-zinc-800 p-4 rounded-xl border border-zinc-700 flex justify-between items-center">
              <div>
      ...
      <section>
        <h2 className="text-xl font-semibold mb-4">Create a New Semester</h2>
