...
// --- Fetch parking data from DB ---
app.get("/api/parking", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM parking_status ORDER BY deck_name");
        console.log("🚗 Returning", result.rows.length, "rows from parking_status");
        res.json(result.rows);
    } catch (err) {
        console.error("Error fetching parking data:", err);
        res.status(500).send("Failed to fetch parking data");
    }
});

// --- Refresh parking data: run scraper + update DB ---
app.post("/api/refresh-parking", (req, res) => {
    exec("python3 scraper.py", (err, stdout, stderr) => {
        if (err) {
            console.error("Scraper error:", stderr);
            return res.status(500).send("Scraper failed");
        }

        // After scraping, fetch updated data
        pool.query("SELECT * FROM parking_status ORDER BY deck_name")
            .then(result => res.json(result.rows))
            .catch(dbErr => {
                console.error("Error fetching after refresh:", dbErr);
                res.status(500).send("Database error after scraper");
            });
    });
});
